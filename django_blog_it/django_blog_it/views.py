...
def active_admin_required(view_func):
    decorated_view_func = login_required(
        admin_required(view_func), login_url='/')
...
class PostDeleteView(PostAccessRequiredMixin, DeleteView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.POST.get("action") == "trash":
            ...
            messages.success(
                request,
                'Blog "' + str(blog_post.title) +
                '" has been restored from trash.'
            )
            return HttpResponseRedirect(self.success_url)
...
class CategoryStatusUpdateView(AdminOnlyMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        category = get_object_or_404(
            Category, slug=kwargs.get("category_slug"))
    ...
    def get(self, request, *args, **kwargs):
        if 'blog_ids[]' in request.GET:
            ...
            if request.GET.get('action') in [status[0] for status in STATUS_CHOICE]:
                ...
                messages.success(
                    request, "successfully updated status to " + request.GET.get('action'))
            ...
            messages.warning(
                request, 'Please select a record to perform action')
    ...
    def get(self, request, *args, **kwargs):
        if 'blog_ids[]' in request.GET:
            if request.GET.get('action') == 'True':
                Category.objects.filter(id__in=request.GET.getlist(
                    'blog_ids[]')).update(is_active=True)
                messages.success(
                    request, 'Selected Categories successfully updated as Active')
            elif request.GET.get('action') == 'False':
                Category.objects.filter(id__in=request.GET.getlist(
                    'blog_ids[]')).update(is_active=False)
                messages.success(
                    request, 'Selected Categories successfully updated as Inactive')
            elif request.GET.get('action') == 'Delete':
                Category.objects.filter(
                    id__in=request.GET.getlist('blog_ids[]')).delete()
                messages.success(
                    request, 'Selected Categories successfully deleted!')
            return JsonResponse({'response': True})
        else:
            messages.warning(
                request, 'Please select a record to perform this action')
...
class UserUpdateView(AdminOnlyMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, 'Successfully Updated User "' + str(user) + '"')
...
class UserBulkActionsView(AdminOnlyMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        if 'user_ids[]' in request.GET:
            if request.GET.get('action') == 'True':
                User.objects.filter(id__in=request.GET.getlist(
                    'user_ids[]')).update(is_active=True)
                messages.success(
                    request, 'Selected Users successfully updated as Active')
            elif request.GET.get('action') == 'False':
                User.objects.filter(id__in=request.GET.getlist(
                    'user_ids[]')).update(is_active=False)
                messages.success(
                    request, 'Selected Users successfully updated as Inactive')
            elif request.GET.get('action') == 'Delete':
                User.objects.filter(
                    id__in=request.GET.getlist('user_ids[]')).delete()
                messages.success(
                    request, 'Selected Users successfully deleted!')
            return JsonResponse({'response': True})
        else:
            messages.warning(
                request, 'Please select a record to perform the action')
            return JsonResponse({'response': False})
    ...
    def get(self, request, *args, **kwargs):
        if 'theme_ids[]' in request.GET:
            if request.GET.get('action') == 'False':
                Theme.objects.filter(id__in=request.GET.getlist(
                    'theme_ids[]')).update(enabled=False)
                messages.success(
                    request, "Selected Theme's successfully updated as Disabled")
            elif request.GET.get('action') == 'Delete':
                Theme.objects.filter(
                    id__in=request.GET.getlist('theme_ids[]')).delete()
                messages.success(
                    request, "Selected Theme's successfully deleted!")
            return JsonResponse({'response': True})
        else:
            messages.warning(
...
def google_login(request):
    if 'code' in request.GET:
        ...
        picture = user_document['picture'] if 'picture' in user_document.keys(
        ) else ""
        ...
        rty = "https://accounts.google.com/o/oauth2/auth?client_id=" + \
            os.getenv("GP_CLIENT_ID") + "&response_type=code"
...
def facebook_login(request):
    if 'code' in request.GET:
        ...
        accesstoken = graph.extend_access_token(
            os.getenv("FB_APP_ID"), os.getenv("FB_SECRET"))['accesstoken']
        hometown = profile['hometown']['name'] if 'hometown' in profile.keys(
        ) else ''
        location = profile['location']['name'] if 'location' in profile.keys(
        ) else ''
        ...
        if 'email' in profile.keys():
            ...
            message.error(
                request, "Sorry, We didnt find your email id through facebook")
            return render(request, '404.html')
    elif 'error' in request.GET:
        print(request.GET)
    else:
        rty = "https://graph.facebook.com/oauth/authorize?client_id=" + os.getenv("FB_APP_ID") + "&redirect_uri=" + 'https://' + request.META['HTTP_HOST'] + reverse(
