...
from .django_blog_it_en.views import *
from .posts.views import *
from .settings import MEDIA_URL, MEDIA_ROOT


urlpatterns = [
    path('', Home.as_view(), name='index_en'),
    path('contacto', feedback, name='feedback_en'),
    path('categoria/<slug:category_slug>',
         SelectedCategoryView.as_view(), name='selected_category_en'),
    re_path(r'^(?P<year>\w{0,})/(?P<month>\w{0,})/$',
            ArchiveView.as_view(), name='archive_posts_en'),
    path('<slug:blog_slug>', BlogPostView.as_view(), name='blog_post_view_en'),
    path('procurar/', blog_SearchView.as_view(), name='blog_search_en'),


    path('dashboard/', AdminLoginView.as_view(), name='admin_login_en'),
    ...
    path('dashboard/category/edit/<slug:category_slug>/',
         CategoryUpdateView.as_view(), name='edit_category'),
    path('dashboard/category/delete/<slug:category_slug>/',
    ...
    path('dashboard/user/edit/<slug:pk>/',
         UserUpdateView.as_view(), name='edit_user'),
    path('dashboard/user/update/<slug:pk>/',
         user_status_update, name='user_status_update'),
